...
읽는 길이는 클라이언트가 준 `attrlen`이 결정하고, 토큰들이 실제로 덮는 구간과 비교해
검증한다 — 이 비교가 경계 밖 읽기를 막는 장치다. 불일치하면 선언 길이가 실제와 다르다는
에러를 낸다.

> **정정 — 구현에서 뒤집힘.** 이 절은 ATTR JSON에 공백을 허용하고, 토큰들이 덮는 구간을
> 원시 포인터로 읽어 NUL을 공백으로 되돌리는 방식으로 설계했다. 구현에서는 그 복원을
> 없애고 **JSON을 인자 하나로 받는다.** 이유는 두 가지다.
>
> 1. 복원이 모호하다. 되돌려야 하는 NUL은 클라이언트가 보낼 수도 있는 바이트다.
> 2. 공백 예산이 사라지면서 128바이트 전 구간을 쓸 수 있게 됐다 — 아래 표의 어떤 행보다
>    넉넉하다.
>
> 토큰 한계를 확장이 스스로 거부하도록 한 설계도 도달 불가능했다.
> `process_extension_command`가 `accept`를 부르기 전에 `ntokens == MAX_TOKENS`를
> 거부한다(`memcached.c:10189`). 해당 검사는 제거했다.
>
> 아래 표는 공백을 허용하던 시절의 실측 기록으로 남긴다.

**공백 개수에는 상한이 있었다.** 토큰 수가 `MAX_TOKENS`(30)에 닿으면 토크나이저가 남은
줄을 쪼개지 않고 통째로 남긴다.

| JSON 스타일 | 예 | 결과 |
|---|---|---|
| 공백 없음 | `{"k0":0,"k1":1,...}` | 128B까지 통과 |
| `json.dumps`·`jq` 스타일 (쉼표 뒤 1칸) | `{"k0":0, "k1":1, ...}` | 14개 필드 120B까지 통과 |
| 모든 토큰 사이에 공백 | `{ "k0" : 0 , "k1" : 1 , ... }` | 7개 필드(28토큰)까지 통과 |

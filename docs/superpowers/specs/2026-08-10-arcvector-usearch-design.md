...
**공백 개수에는 상한이 있다.** 토큰 수가 `MAX_TOKENS`(30)에 닿으면 토크나이저가 남은 줄을
쪼개지 않고 통째로 남기는데, 그 길이는 확장이 받은 토큰 배열 범위를 벗어난 슬롯에 있어
구간을 검증할 수 없다. 그래서 이 경우는 추측하지 않고 거부한다. 실측한 경계는 다음과 같다.

| JSON 스타일 | 예 | 결과 |
|---|---|---|
| 공백 없음 | `{"k0":0,"k1":1,...}` | 128B까지 통과 |
| `json.dumps`·`jq` 스타일 (쉼표 뒤 1칸) | `{"k0":0, "k1":1, ...}` | 14개 필드 120B까지 통과 |
| 모든 토큰 사이에 공백 | `{ "k0" : 0 , "k1" : 1 , ... }` | 6개 필드 67B에서 31토큰 → 거부 |

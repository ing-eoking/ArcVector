...
```
 off  0    2   3     4       6        8       16          16+F
     +----+---+-----+-------+--------+-------+-----------+---------------+
     |"AV"|ver|quant|dim u16|flen u16|rsvd 8B| 필터 슬롯 F | 양자화 벡터     |
     +----+---+-----+-------+--------+-------+-----------+---------------+
       헤더 16B (고정)                          고정 F       dim*sz(quant)
```
...
| flen | 2B LE | 필터 슬롯 내 실제 JSON 길이 |
| rsvd | 8B | 정렬 및 향후 확장 |
| 필터 슬롯 | F | JSON 텍스트, 뒤는 0 패딩 |
| 벡터 | dim×sz | 양자화된 좌표 |

설계 근거:

- **필터 슬롯이 벡터 앞에 온다.** 오프셋이 `dim`·`quant`와 무관한 상수 16이므로
  predicate가 캐시라인 하나만 건드린다.
- **헤더 16B + F는 16의 배수.** 벡터 시작 오프셋이 16 정렬되어 SIMD에 유리하다.
- `F` 기본값 **64B** — 캐시라인 정확히 1개. `{"cat":"tech","lang":"ko","ts":1723248000}` = 44B로
  실사용 필터가 들어간다. `vcreate ... FBYTES n`으로 16B 배수, 16~256 범위 조정.
...
`max_element_bytes = 16KB`, `F = 64` 기준 최대 차원:

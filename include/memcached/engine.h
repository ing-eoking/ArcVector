...
#define LAST_REGISTERED_ENGINE_FEATURE ENGINE_FEATURE_VBUCKET
    ...
    typedef struct engine_interface_v1 {
         ...
         * @return ENGINE_SUCCESS if all goes well
         */
        ENGINE_ERROR_CODE (*rp_cmd)(ENGINE_HANDLE* handle,
                                    const char* cmd, const char* param);
        ENGINE_ERROR_CODE (*rp_standalone)(ENGINE_HANDLE *handle);
        ENGINE_ERROR_CODE (*rp_master_mode)(ENGINE_HANDLE *handle,
                                            const char *listen_ipport);
        ENGINE_ERROR_CODE (*rp_slave_addr)(ENGINE_HANDLE *handle,
                                           rpaddr_t *slave_addrs, int slave_count);
        ENGINE_ERROR_CODE (*rp_slave_mode)(ENGINE_HANDLE *handle,
                                           const char *listen_ipport,
                                           rpaddr_t *master_addr, int slave_count);
        int  (*rp_max_replicas)(ENGINE_HANDLE* handle);
        bool (*rp_master_in_switchover)(ENGINE_HANDLE* handle);
#endif

#ifdef ENABLE_MIGRATION
        /**
         * Commands to the migration module.
         *
         * @param handle the engine handle
         *
         * @return ENGINE_SUCCESS if all goes well
         */
        ENGINE_ERROR_CODE (*mg_prepare)(ENGINE_HANDLE* handle, const char *self_ipport,
                                        bool join, hslice_t *hs_array, uint8_t *hs_valid);
        ENGINE_ERROR_CODE (*mg_hranges)(ENGINE_HANDLE* handle,
                                        hslice_t *hs_array, uint8_t *hs_valid);
        ENGINE_ERROR_CODE (*mg_start)(ENGINE_HANDLE *handle,
                                      const char *peer_ipport, const char *peer_name,
                                      int hs_index, int hs_count, hslice_t *hs_range);
        void              (*mg_stop)(ENGINE_HANDLE *handle, bool success);
        void              (*mg_done)(ENGINE_HANDLE *handle);
        /**
         * Cehck engine scrubber is in running.
         *
         * @param handle the engine that owns the object
         *
         * @return true if scrubber is in running
         */
        bool (*scrubber_in_running)(ENGINE_HANDLE *handle);
#endif

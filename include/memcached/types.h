...
/************************/
/* EE version code tags */
/************************/
#define AUTO_LEAVE_ABORT
#define ENABLE_DUMP_RCOUNT
#define CSET_VERSION
#define ENABLE_DB_ENGINE
#define SUPPORT_DB_COLL
#define REPL_RESEND
...
#ifdef ENABLE_DB_ENGINE
        ENGINE_PREFIX_EEXIST = 0x53,
        ENGINE_PREFIX_EOVERFLOW = 0x54,
#endif

#ifdef ENABLE_REPLICATION
        ENGINE_REPL_SLAVE = 0x61,      /**< Write requests on slave node */
        ENGINE_SWITCHOVER_WAIT = 0x62, /**< Switchover is going on. Block requests. */
        ENGINE_SWITCHOVER_DONE = 0x63, /**< Switchover has been done */
#endif
#ifdef ENABLE_MIGRATION
        ENGINE_NOT_MY_KEY   = 0x64, /**< Given key is not mine. Migration might have done. */
#endif
#ifdef ENABLE_DB_ENGINE
        ENGINE_ENOCONFFILE = 0x70,
        ENGINE_ENODISK     = 0x71,
        ENGINE_ETRANS      = 0x72,
        ENGINE_EINCONSISTENT = 0x73,
        ENGINE_PAGE_FAILED = 0x74,
        ENGINE_INDX_FAILED = 0x75,
        ENGINE_BUFF_FAILED = 0x76,
#endif
...
#ifdef ENABLE_REPLICATION
    /* replication address structure */
    typedef struct {
        char   *mcaddr; /* mc service <ip:port> string */
        char   *ipport; /* replication <ip:port> string */
        int32_t seqnum; /* ZK sequence number */
    } rpaddr_t;
#endif

#ifdef ENABLE_MIGRATION
    /* hash slice structure */
    typedef struct {
        uint32_t spoint; /* start hash point */
        uint32_t epoint; /* end   hash point */
    } hslice_t;
#endif
